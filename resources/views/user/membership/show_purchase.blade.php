                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <p class="mb-1"><strong>ประเภทบัตร:</strong> {{ $purchase->membershipTier->tier_name }}
                                </p>
                                <p class="mb-1"><strong>วันที่สั่งซื้อ:</strong>
                                    {{ thaidate('j F Y, H:i น.', $purchase->created_at) }}</p>
                            </div>
                        ...
                        <div class="mt-4">
                            ...
                            @if ($purchase->status == 'pending_payment')
                                ...
                                <div class="alert alert-secondary">
                                    <p class="mb-1">กรุณาชำระเงินจำนวน <strong
                                            class="fs-4 text-danger">{{ number_format($purchase->price, 2) }}</strong> บาท
                                    </p>
                                    <p class="small mb-0">มาที่บัญชี: ธ.กสิกรไทย 255-1-03447-2 (สหกรณ์อิสลามษะกอฟะฮ จำกัด)
                                    </p>
                                </div>
                                <form action="{{ route('user.purchase.uploadSlip', $purchase) }}" method="POST"
                            ...
                            @elseif($purchase->status == 'verifying')
                                ...
                                <p class="text-muted">เราได้รับข้อมูลการชำระเงินของคุณแล้ว และกำลังดำเนินการตรวจสอบ
                                    โปรดรอการยืนยันจากเจ้าหน้าที่</p>
                                <div class="text-center slip-preview">
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#viewSlipModal">
                                        <img src="{{ Storage::url($purchase->slip_image_path) }}" class="img-fluid rounded"
                                            alt="Payment Slip">
                                    </a>
                                </div>
                            @elseif($purchase->status == 'completed')
                                <div class="alert alert-success text-center">
                                    ...
                                    <p class="mb-0">บัตรสมาชิกของคุณถูกเปิดใช้งานเรียบร้อยแล้ว
                                        สามารถตรวจสอบได้ที่หน้าแดชบอร์ด</p>
                                </div>
                            @elseif($purchase->status == 'rejected')
                                <div class="alert alert-danger">
                                    <h5>รายการถูกปฏิเสธ</h5>
                                    <p class="mb-0"><strong>เหตุผล:</strong>
                                        {{ $purchase->rejection_reason ?? 'กรุณาติดต่อเจ้าหน้าที่' }}</p>
                                </div>
                            @endif
                        </div>
                        <div class="mt-4 text-center">
                            <a href="{{ route('user.dashboard') }}" class="btn btn-outline-secondary">&laquo;
                                กลับสู่แดชบอร์ด</a>
                        </div>
                    </div>
                </div>
